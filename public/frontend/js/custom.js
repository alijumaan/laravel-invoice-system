$(function () {
    ...
    $('#invoice_details').on('keyup blur', '.discount_type, .discount_value, .shipping', function () {
        $('#vat_value').val(calculate_vat());
        $('#total_due').val(sum_due_total());
    });
    ...
    let calculate_vat = function () {
        ...
        // This Code Is Short Than If Last Statement
    ...
    $(document).on('click', '.btn_add', function () {
        ...
        $('#invoice_details').find('tbody').append($('' +
            ...
            '<td><input type="text" name="product_name[' + numberIncr + ']" class="form-control product_name"></td>' +
            '<td><select name="unit[' + numberIncr + ']" class="form-control unit"><option></option><option value="piece">Piece</option><option value="g">Gram</option><option value="k">Kilo_gram</option></select></td>' +
        '<td><input type="number" step="0.01" name="quantity[' + numberIncr + ']" class="form-control quantity"></td>' +
        '<td><input type="number" step="0.01" name="unit_price[' + numberIncr + ']" class="form-control unit_price"></td>' +
        '<td><input type="number" step="0.01" name="row_sub_total[' + numberIncr + ']" class="form-control row_sub_total" readonly="readonly"></td>' +
        '</tr>'));
    });
